...
class main:
    ...
    def handle_response_errors(self, response: Response):
        if (response.status_code == 401): #401 status code means invald authorization.
    ...
    def retrive_game_endpoints(self):
        ...
        while not end_reached:
            if i == 0:  
                content = json.loads(self.message_request(progress_anouncements.id,"?limit=100").text)
            else:
                content = json.loads(self.message_request(progress_anouncements.id, f"?before={last_message}&limit=100").text)
            i += 1
    ...
    def retrive_game(self, start_date: datetime, end_date: datetime):
        ...
        total = 0 #Need to initize this variable before it is asigned a value.
        offset = 0
        i = 0
        while total >= 0:
            if i == 0: #First iteration doesn't need an offset.
                r = json.loads(self.message_request_between(server_id, start, end, progress_anouncements.id).text)
                total = r["total_results"]
            else: #Second iteration does, however.
                r = json.loads(self.message_request_between(server_id, start, end, progress_anouncements.id, f"&offset={offset}").text)
        ...
        for idx, (key, team) in enumerate(all_player_channels.items()):
            ...
            for channel in team:
                ...
                while total >= 0:
                    if i == 0: #First iteration doesn't need an offset.
                        r = json.loads(self.message_request_between(server_id, start, end, channel.id).text)
                        total = r["total_results"]
                    else: #Second iteration does, however.
